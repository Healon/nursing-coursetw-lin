...
Output: run_all() -> (events, outcomes)。每個被執行的來源都有明確 outcome，失敗不中斷其他來源。
"""
...
def run_all(only: list[str] | None = None) -> tuple[list[dict], dict[str, dict]]:
    """Run source parsers.

    only=None 時跑所有 enabled 來源；指定 only 時無視 enabled（方便單獨測一個來源）。
