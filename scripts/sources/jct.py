...
過濾掉，不產出事件。排除規則已 config 化：`SOURCES['jct']['exclude_title_keywords']`
（現含 數位課程／教學影片／觀摩活動），維護者往清單加詞即可，見 `_exclude_by_title_rules`。
...
def _exclude_keywords() -> list[str]:
    """讀取 config 的排除關鍵字清單（維護者可自行增減，不用改程式）。"""
    return list(cfg.SOURCES.get("jct", {}).get("exclude_title_keywords", []))


def _exclude_by_title_rules(candidates: list[dict]) -> list[dict]:
    """把標題命中排除關鍵字（config SOURCES['jct']['exclude_title_keywords']）的候選整批排除。

    Lin 指示的排除規則（不收錄上站）：數位課程／教學影片（2026-07-10，本站定位為排定場次
    彙整）、觀摩活動（2026-07-10，NHQA 現場發表觀摩非課程）。日後再有新規則，往 config
    清單加關鍵字即可。放在去重後、選詳情前這個位置過濾最省請求——命中的候選連詳情頁都
    不必抓。排除以「關鍵字：筆數」彙總形式 stderr 留痕，不可無聲丟棄（靜默失敗必須可見）。
    """
    keywords = _exclude_keywords()
    if not keywords:
        return candidates
    kept: list[dict] = []
    hit_counts: dict[str, int] = {}
    for c in candidates:
        hit = next((kw for kw in keywords if kw in c["title"]), None)
        if hit is None:
            kept.append(c)
        else:
            hit_counts[hit] = hit_counts.get(hit, 0) + 1
    if hit_counts:
        detail = "、".join(f"{kw} {n} 筆" for kw, n in hit_counts.items())
        print(f"[jct] 依排除規則略過：{detail}（config exclude_title_keywords）", file=sys.stderr)
    return kept
...
def fetch() -> list[dict]:
    ...
    candidates = _exclude_by_title_rules(_dedupe_candidates(all_candidates))
